...
class ReceivablePayableReport(object):
	...
	def init_voucher_balance(self):
		...
		for gle in self.gl_entries:
			...
			if not key in self.voucher_balance:
				self.voucher_balance[key] = frappe._dict(
					...
					customer_branch=self.filters.customer_branch if self.filters.customer_branch else ''
				)
		...
		frappe.msgprint(_("{0}").format(self.voucher_balance))
	...
	def build_data(self):
		...
		for key, row in self.voucher_balance.items():
			#frappe.msgprint(_("{0}, {1}").format(key, row))
	...
	def append_row(self, row):
		#frappe.msgprint(_("{0}").format(row))
	...
	def get_invoice_details(self):
		...
		if self.party_type == "Customer":
			si_list = frappe.db.sql(
				"""
				select name, due_date, po_no, customer_branch
			...
			# Customer branch
			if self.filters.customer_branch:
				si_list = frappe.db.sql(
					"""
						select name, due_date, po_no, customer_branch
						from `tabSales Invoice`
						where posting_date <= %s and customer_branch = %s
					""",
						(self.filters.report_date, self.filters.customer_branch), 
						as_dict=1,
				)
				#frappe.msgprint(_("{0}").format(si_list))
				for d in si_list:
					self.invoice_details.setdefault(d.name, d)
		#frappe.msgprint(_("{0}").format(self.invoice_details))
	...
	def get_gl_entries(self):
		...
		sql = ("""select name, posting_date, account, party_type, party, voucher_type, voucher_no, cost_center, against_voucher_type, against_voucher, account_currency, {0}, {1} {remarks} from `tabGL Entry` where docstatus < 2 and is_cancelled = 0 and party_type=%s and (party is not null and party != '') {2} {3} {4}""".format(select_fields, doc_currency_fields, date_condition, conditions, order_by, remarks=remarks), values)

		frappe.msgprint(_("{0}").format(sql))
		...
		frappe.msgprint(_("{0}, {1}").format(self.gl_entries, conditions))
	...
	def prepare_conditions(self):
		...
		if self.filters.customer_branch:
			conditions.append("customer_branch in ('{0}')".format(self.filters.customer_branch))
	...
	def get_columns(self):
		...
		if self.party_type == "Customer":
			...
			self.add_column(_("Customer Branch"), fieldname="customer_branch", fieldtype="Link", options="Customer Branch", width=180)
